...
<?php
    ...
    if($custid != 0)
    {
        ...
        sendmail($rscustomer['email_id'], $rscustomer['customer_name'] , "ZimCrafts Artist's Kit Invoice", $mailcontent);
    }
    if($seller_id != 0)
    {
        $sqlseller = "SELECT * FROM seller WHERE seller_id='" . $seller_id . "'";
        $qsqlseller = mysqli_query($con,$sqlseller);
        $rsseller = mysqli_fetch_array($qsqlseller);
        sendmail($rsseller['email_id'], $rsseller['seller_name'] , "ZimCrafts Artist's Kit Invoice", $mailcontent);
    }
    //Mail Code Ends here
