...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		if(!mysqli_query($con,$sql)){
			...
			$sqlseller = "SELECT * FROM seller WHERE seller_id='$insid'";
			$qsqlseller = mysqli_query($con,$sqlseller);
			$rsseller = mysqli_fetch_array($qsqlseller);
			include("phpmailer.php");
			$protocol = strtolower(substr($_SERVER["SERVER_PROTOCOL"],0,5))=='https'?'https':'http';
			$protocol = isset($_SERVER["HTTPS"]) ? 'https' : 'http';
			$registrationmsg = "Hello " . $rsseller['seller_name'] . "<br><br>Greetings from ZimCrafts Marketplace!<br><br>Thank you for signing up! We're excited to have you here. Sign into your account and start selling at ZimCrafts...<br><br> ". "<a href='" . $protocol . "://" . $_SERVER['SERVER_NAME'].substr($_SERVER['PHP_SELF'], 0, strrpos($_SERVER['PHP_SELF'], '/')) . "/sellerloginpanel.php' >Click here to Login</a>";
			sendmail($rsseller['email_id'], $rsseller['seller_name'] , "Welcome to ZimCrafts Seller", $registrationmsg);
...
<hr>
	...
	?>
	<button type="button" name="submitotp" id="submitotp" class="btn btn-info btn-lg btn-block"  onclick="return validatesellreg()" >Click here to Register</button>
	<?php
	}
	else {
	?>
	<button type="submit" name="submit" id="submit" class="btn btn-info btn-lg btn-block"  onclick="return validatesellreg()" >Click here to Register</button>
	<?php
	}
	?>
<script>
	  ...
	  <div class="modal-dialog" style="max-width: 50%;">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	      	<button name="submit" id="submit" class="btn btn-info button-login" type="submit" onclick="return validateotp()">Complete Registration</button>
	      </div>
	    </div>
	  </div>
	</div>  
...
?>
	...
	else if(document.frmsellreg.cpassword.value == "")
	{
		    ...
		    else if(!document.frmsellreg.branch.value.match(alphaspaceExp))
		{
		  ...
		  {
			alert("Kindly enter the Swift Code of the bank...");
		  ...
		  {
			alert("Kindly enter a valid 8-11 Characters Zimbabwean Bank Swift Code...");
			document.frmsellreg.ifsccode.focus();
			return false;
		  }
		  ...
		  else
		  {
				...
				xmlhttp.open("GET","sendotp.php?mblnum="+document.getElementById("mbnumber").value+"&cstname="+document.getElementById("sellername").value,true);
				xmlhttp.send();
		  }
	   }
