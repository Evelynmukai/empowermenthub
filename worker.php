...
if($_SESSION['randnumber']  == $_POST['randnumber'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sql="INSERT INTO worker( name, address, state_id, city_id, country_id, pincode, contactno, work_profile, biodata, date_of_birth, login_id, password, expected_salary, status) VALUES ('" . mysqli_real_escape_string($con,$_POST['name']) . "','" . mysqli_real_escape_string($con,$_POST['address']) . "','$_POST[state]','$_POST[city]','$_POST[country]','$_POST[pincode]','$_POST[contct]','$_POST[workprofile]','$imgname1','$_POST[dob]','$_POST[loginid]','$_POST[password]','$_POST[expectedsalary]','Active')";
			if(!mysqli_query($con,$sql))
			{
				echo "Error in mysqli query";
			}
...
<hr>
	...
	?>
		<button type="button" name="submitotp" id="submitotp" class="btn btn-info btn-lg btn-block"  onclick="return validateworkreg()" >Click here to Register</button>
	<?php
	}
	else {
	?>
	<button type="submit" name="submit" id="submit" class="btn btn-info btn-lg btn-block" onclick="return validateworkreg()" >Click here to Register</button>
	<?php } ?>
<script>
	  ...
	  <div class="modal-dialog" style="max-width: 50%;">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	      	<button value="Login" name="submit" id="submit" class="btn btn-info button-login" type="submit" onclick="return validateotp()">Complete Registration</button>
	      </div>
	    </div>
	  </div>
	</div>
